...
class Model(nn.Module):
    def __init__(self, classes, classes_map, args):
        ...
        self.lower_rate_epoch = [
            int(0.7 * self.num_epochs),
            int(0.9 * self.num_epochs),
        ]  # hardcoded decay schedule
    ...
    def classify(self, images):
        ...
        """
        _, preds = torch.max(
            torch.softmax(self.forward(images), dim=1), dim=1, keepdim=False
        )

        return preds

    # def update(self, dataset, class_map, args):
    def update(self, loader, class_map, args):
        ...
        new_classes = [i for i in range(classes)]
        ...
        print("Batch Size (for n_classes classes) : ", 32)
        optimizer = optim.SGD(
            self.parameters(),
            lr=self.init_lr,
        ...
        with tqdm(total=self.num_epochs) as pbar:
            for epoch in range(self.num_epochs):
                ...
                for i, (images, labels) in enumerate(loader):
                    ...
                    seen_labels = torch.LongTensor(
                        [class_map[label] for label in labels.numpy()]
                    )
                    ...
                    if self.n_classes // len(new_classes) > 1:
                        ...
                        logits_dist = logits[:, : ]
                        # print(logits_dist.shape, dist_target.shape)
                    ...
                    if (i + 1) % (len(loader.dataset) // 10) == 0:
                        tqdm.write(
                            ...
                            % (
                                ...
                                np.ceil(len(loader.dataset) / self.batch_size),
                                loss.data,
                            )
                        )
