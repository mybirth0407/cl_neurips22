...
class DEN(object):
    def __init__(self, config):
        ...
        self.early_training = config.max_iter / 10.0
        ...
        if "layer%d" % self.n_layers == scope:
            ...
            for i in self.task_indices:
                if i == self.T:
                    ...
                    new_w = tf.get_variable(
                        "new/n%d" % i, [ex_k, self.n_classes], trainable=True
                    )
                else:
            ...
            expanded_n_units = self.expansion_layer[
                self.n_layers - level - 2
            ]  # top-down

            next_dim = w.get_shape().as_list()[1]
            new_w = tf.get_variable(
                scope + "new_tw", [self.ex_k, next_dim], trainable=True
            )
    ...
    def extend_param(self, scope, ex_k):
        if "layer%d" % self.n_layers == scope:
            ...
            new_w2 = self.create_variable(
                scope + "/new_fc", "tw", [ex_k, next_dim + ex_k]
            )
    ...
    def build_model(
        ...
        if splitting:
            for i in range(1, self.n_layers):
                ...
                expanded_b = np.concatenate(
                    (
                        ...
                        np.random.rand(len(new_ary)),
                    )
                ).astype(np.float32)
                with tf.variable_scope("layer%d" % i):
                    w = tf.get_variable(
                        "weight", initializer=expanded_w, trainable=True
                    )
                    b = tf.get_variable(
                        "biases", initializer=expanded_b, trainable=True
                    )
                self.params[w.name] = w
                self.params[b.name] = b
            ...
            w = self.get_variable(
                "layer%d" % self.n_layers, "weight_%d" % task_id, False
            )
            b = self.get_variable(
                "layer%d" % self.n_layers, "biases_%d" % task_id, False
            )
    ...
    def selective_learning(self, task_id, selected_params):
        ...
        for i in range(1, self.n_layers):
            with tf.variable_scope("layer%d" % i):
                w = tf.get_variable(
                    "weight",
                    initializer=selected_params["layer%d/weight:0" % i],
                )
                b = tf.get_variable(
                    "biases",
                    initializer=selected_params["layer%d/biases:0" % i],
                )
            bottom = tf.nn.relu(tf.matmul(bottom, w) + b)
        # last layer
        with tf.variable_scope("layer%d" % self.n_layers):
            w = tf.get_variable(
                "weight_%d" % task_id,
                initializer=selected_params[
                    "layer%d/weight_%d:0" % (self.n_layers, task_id)
                ],
            )
            b = tf.get_variable(
                "biases_%d" % task_id,
                initializer=selected_params[
                    "layer%d/biases_%d:0" % (self.n_layers, task_id)
                ],
            )
    ...
    def optimization(
        ...
        if selective:
            all_var = [
                ...
                if "layer%d" % self.n_layers in var.name
            ]
        else:
        ...
        losses = (
            self.loss
            + self.l2_lambda * tf.reduce_sum(l2_losses)
        ...
        with tf.control_dependencies([apply_grads]):
            for var in l1_var:
                ...
                l1_op = var.assign(
                    tf.where(tf.less(tf.abs(var), th_t), zero_t, var_temp)
                )
        ...
        GL_var = [
            var
            for var in tf.trainable_variables()
            if "new" in var.name and ("bw" in var.name or "tw" in var.name)
        ]
        gl_op_list = []
        with tf.control_dependencies([apply_grads]):
            for var in GL_var:
                ...
                for i in range(var.get_shape()[1]):
                    ...
                    gw_gl = tf.where(
                        tf.less(g_sum[i], th_t),
                        tf.zeros(tf.shape(var[:, i])),
                        temp_gw,
                    )
                    gw.append(gw_gl)
    ...
    def set_initial_states(self, decay_step):
        self.g_step = tf.Variable(0.0, trainable=False)
    ...
    def add_task(self, task_id, data):
        ...
        if self.T == 1:
            ...
            for i in range(self.n_layers, 0, -1):
                if i == self.n_layers:
                    ...
                    selected_params["layer%d/weight_%d:0" % (i, task_id)] = w[
                    ...
                    selected_prev_params["layer%d/weight:0" % i] = self.prev_W[
                        "layer%d/weight:0" % i
                    ][np.ix_(all_indices["layer%d" % i], top_indices)]
                    selected_prev_params["layer%d/biases:0" % i] = self.prev_W[
                        "layer%d/biases:0" % i
            ...
            self.sess.run(tf.global_variables_initializer())
            repeated, c_loss = self.run_epoch(
                self.opt, self.loss, trainX, trainY, "Train", print_pred=False
            )
            _vars = [
                (var.name, self.sess.run(var))
                for var in tf.trainable_variables()
            ...
            if c_loss < self.loss_thr:
                ...
                for i in range(self.n_layers - 1, 0, -1):
                    ...
                    useless = []
                    for j in range(
                        prev_layer_weight.shape[1] - self.ex_k,
                        prev_layer_weight.shape[1],
                    ):
                    ...
                    print(
                        "   [*] Expanding %dth hidden unit, %d unit added, (valid, repeated: %d)"
                ...
                for i in range(1, self.n_layers):
                    ...
                    print(
                        "   [*] split N in layer%d: %d / %d"
                        % (i, len(_temp), len(cosims))
                    )
        ...
        self.sess.run(tf.global_variables_initializer())
        test_preds, test_loss = self.sess.run(
            [self.yhat, self.loss], feed_dict={self.X: testX, self.Y: testY}
        )
        ...
        print(
            " [*] Task: %d, nn_test_loss: %.4f, test_perf: %.4f, sparsity(avg): %.4f"
    ...
    def run_epoch(
        ...
        while self.max_iter > c_iter:
            ...
            _, c_loss = self.sess.run(
                [opt, loss], feed_dict={self.X: batch_X, self.Y: batch_Y}
            )
            c_iter += 1
            print_iter = 100
            ...
            if desc == "Train" and c_iter % print_iter == 0:
                val_preds, val_loss = self.sess.run(
                    [self.yhat, loss],
                    feed_dict={self.X: self.valX, self.Y: self.valY},
                )
                ...
                if print_pred == True:
                    print(
                        " [*] iter: %d, val loss: %.4f, val perf: %.4f"
